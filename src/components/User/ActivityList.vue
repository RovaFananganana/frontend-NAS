  ...
  <div class="activity-list">
    ...
    <div v-else-if="!loading && activities.length === 0" class="empty-state">
      ...
      <p class="empty-description">
        {{ hasFilters ? 'Aucune activité ne correspond aux filtres sélectionnés.' : 'Vous n\'avez pas encore d\'activité
        enregistrée.' }}
      </p>
      <button v-if="hasFilters" @click="$emit('clear-filters')" class="clear-filters-btn">
    ...
    <div v-else class="activities-container">
      ...
      <div class="activities-list">
        <div v-for="activity in activities" :key="activity.id" class="activity-item"
          :class="{ 'activity-failed': !activity.success }" @mouseenter="showTooltip(activity, $event)"
          ...
          <div class="activity-status">
            <span v-if="activity.success" class="status-success" title="Action réussie">
              <i class="fas fa-check-circle"></i>
            </span>
            <span v-else class="status-failed" title="Action échouée">
              <i class="fas fa-times-circle"></i>
            </span>
          </div>
        </div>
    ...
    <!-- Tooltip -->
    <div v-if="tooltip.show" ref="tooltipRef" class="activity-tooltip" :style="tooltip.style">
...
.activity-item {
  @apply flex items-start gap-4 p-4 rounded-lg border border-gray-200 hover:border-gray-300 hover:shadow-sm transition-all duration-200 cursor-pointer;
}
...
.activity-icon {
  @apply relative flex-shrink-0 w-10 h-10 flex items-center justify-center bg-gray-100 rounded-full;
}

.error-indicator {
  @apply absolute -top-1 -right-1 w-4 h-4 bg-white rounded-full flex items-center justify-center;
}
...
.activity-error {
  @apply mt-2 p-2 bg-red-100 border border-red-200 rounded text-sm text-red-700 flex items-start gap-2;
}
...
.load-more-btn {
  @apply px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors duration-200 flex items-center gap-2 mx-auto disabled:opacity-50 disabled:cursor-not-allowed;
}
