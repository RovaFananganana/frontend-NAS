  ...
  <div class="h-full flex bg-base-100">
    ...
    <div class="w-80 border-r border-base-300 bg-base-50 flex flex-col">
        ...
        <div class="form-control">
          <div class="input-group">
            <input v-model="searchQuery" type="text" placeholder="Rechercher par nom..."
              class="input input-bordered input-sm flex-1" @input="onSearch" />
      ...
      <div v-if="!isAdmin && favorites.length > 0" class="p-4 border-b border-base-300">
        ...
        <div class="space-y-1">
          <button v-for="favorite in favorites" :key="favorite.path" @click="navigateToPath(favorite.path)"
      ...
      <div class="flex-1 overflow-auto p-4">
        ...
        <TreeView :selected-path="currentPath" @folder-selected="navigateToPath" />
      </div>
    </div>
    ...
    <div class="flex-1 flex flex-col">
      ...
      <div class="p-4 border-b border-base-300 bg-base-50">
        ...
        <nav class="text-sm breadcrumbs mb-3">
          <ul>
            ...
            <li v-for="(segment, index) in pathSegments" :key="index">
              <button @click="navigateToPath(getPathUpTo(index))" class="btn btn-ghost btn-xs">
                {{ segment }}
              </button>
            </li>
          </ul>
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <button v-if="!isAdmin && currentPath !== '/'" @click="toggleFavorite" class="btn btn-sm btn-outline"
      ...
      <div class="flex-1 overflow-auto p-4">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
          <div v-for="item in filteredItems" :key="item.path" @click="selectItem(item)" @dblclick="openItem(item)"
            ...
            <div class="card-body p-4 text-center">
              ...
              <div v-if="isAdmin" class="mt-2">
                <button @click.stop="openPermissions(item)" class="btn btn-xs btn-outline btn-primary"
    ...
    <div v-if="contextMenu.show"
      ...
      <button v-if="contextMenu.item && !contextMenu.item.is_directory"
        class="w-full text-left px-4 py-2 hover:bg-base-200 text-sm flex items-center gap-3"
      ...
      <!-- Renommer -->
      <button v-if="contextMenu.item && canModifyItem(contextMenu.item)"
        class="w-full text-left px-4 py-2 hover:bg-base-200 text-sm flex items-center gap-3"
      ...
      <!-- Coller -->
      <button v-if="clipboard.items.length > 0 && canModifyCurrentFolder()"
        class="w-full text-left px-4 py-2 hover:bg-base-200 text-sm flex items-center gap-3" @click="pasteItems()">
      ...
      <button v-if="contextMenu.item && isAdmin"
        class="w-full text-left px-4 py-2 hover:bg-base-200 text-sm flex items-center gap-3"
      ...
      <button v-if="contextMenu.item?.is_directory && !isAdmin"
        class="w-full text-left px-4 py-2 hover:bg-base-200 text-sm flex items-center gap-3"
      ...
      <!-- Supprimer -->
      <button v-if="contextMenu.item && canDeleteItem(contextMenu.item)"
    ...
    <!-- Rename Modal -->
    <RenameModal v-if="showRenameModal" :item="itemToRename" @close="showRenameModal = false"
    ...
    <!-- Properties Modal -->
    <PropertiesModal v-if="showPropertiesModal" :item="selectedItemForProperties"
      @close="showPropertiesModal = false" />

    <!-- Download Progress Modal -->
    <div v-if="downloadProgress.show"
