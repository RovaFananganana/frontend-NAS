            ...
            <tbody>
              <tr v-for="log in logs" :key="log.id">
                  ...
                  <div class="flex items-center space-x-2">
                    <div class="avatar">
                      <div class="mask mask-squircle w-8 h-8 bg-secondary text-secondary-content flex items-center justify-center">
                        {{ (log.username || log.user_id || 'S').toString().charAt(0).toUpperCase() }}
                      </div>
                    </div>
                    <div>
                      <div class="font-medium text-sm">{{ log.username || `ID: ${log.user_id}` }}</div>
                    </div>
                  </div>
                </td>
                <td>
                ...
                <td>
                  ...
                  <span v-else class="text-sm">{{ log.target }}</span>
                </td>
              </tr>
            </tbody>
          </table>
...
const getActionLabel = (action) => {
  const labels = {
    ...
    'ADD_USER_TO_GROUP': 'Ajout au groupe',
...
const applyFilters = () => {
  currentPage.value = 1
  // Dans une vraie implémentation, vous enverriez les filtres à l'API
  // Pour l'instant, on recharge simplement les logs
  loadLogs()
}
